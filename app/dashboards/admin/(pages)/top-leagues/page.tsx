...
import { getAdminDashboardTopLeagues } from "@/service/admin/dashboard/admin.dashboard.server.service";

type TopLeaguesPageProps = {
...
export default async function TopLeaguesPage({
    ...
    const leaguesResponse = await getAdminDashboardTopLeagues({
        page,
        limit,
    });

    const leagues = mapAdminDashboardFollowedItems(leaguesResponse.data.items);
    const { meta } = leaguesResponse.data;
    ...
    const getPageHref = (targetPage: number) => {
        ...
        return `${TOP_LEAGUES_ROUTE}?${params.toString()}`;
    };
            ...
            <div className="flex items-start gap-3">
                ...
                <div>
                    <h1 className="text-2xl font-bold lg:text-3xl">
                        Top Followed Leagues
                    </h1>
                    <p className="mt-2 text-sm text-[#6B7A75] dark:text-white/45">
                        View leagues ranked by follower count.
                    </p>
                </div>
            </div>
